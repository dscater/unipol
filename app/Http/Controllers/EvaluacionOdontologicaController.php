...
class EvaluacionOdontologicaController extends Controller
{
    public $titulo = [
        'font' => [
            'bold' => true,
    ...
    public function descargar()
    {
        ...
        $fila = 1;
        $sheet->setCellValue('A' . $fila, "EVALUACIÓN ODONTOLÓGICA");
        ...
        header('Content-Disposition: attachment;filename="evaluacion_odontologica' . time() . '.xlsx"');
    ...
    public function subir(Request $request)
    {
        try {
            ...
            foreach ($filas as $fila) {
                ...
                if (!$postulante->evaluacion_odontologica) {
                    $postulante->evaluacion_odontologica()->create([
                        "valoracion" => mb_strtoupper(trim($colJ)),
                        "nro_baucher" => trim($colK),
                        "nro_folder" => trim($colL),
                    ]);
                } else {
                    $postulante->evaluacion_odontologica->update([
                        "valoracion" => mb_strtoupper(trim($colJ)),
                        "nro_baucher" => trim($colK),
                        "nro_folder" => trim($colL),
                    ]);
